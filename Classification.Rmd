...
overall_bv <- grouped_sim_data %>% 
  select(ID, diagnosis, change_MAD, interaction_time, abs_MAD, mad)

sim_bv <- grouped_sim_data %>% 
  select(ID, diagnosis, abs_MAD, change_MAD, interaction_time)
...
original_acc <- CrossValidation(original_bv, 0.21, 2000)

# Cross-validation for simulated data
sim_acc <- CrossValidation(sim_bv, 0.21, 2000)

# Cross validation for overall data
overall_acc <- CrossValidation(overall_bv, 0.21, 2000)


# Create tibble with mean accuracy scores
accuracy_scores = tibble(
                    data = c("Original", "Simulation", "Overall"),
                    accuracy = c(mean(original_acc$accuracy), mean(sim_acc$accuracy), mean(overall_acc$accuracy)),
                    SD = c(sd(original_acc$accuracy), sd(sim_acc$accuracy), sd((overall_acc$accuracy)))
)
